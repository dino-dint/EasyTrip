...
function Register() {
  ...
  const validate = () => {
    ...
    else if (!/^\d{10,}$/.test(formData.phone.replace(/\D/g, ''))) newErrors.phone = 'Enter a valid phone number'
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sky-100/70 mt-2">Start your journey with EasyTrip</p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-sky-100/80 mb-1.5">Full Name</label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-11 pr-4 py-3 border rounded-xl bg-white/10 text-white placeholder-gray-400 focus:outline-none focus:ring-2 transition ${
                    errors.name ? 'border-red-400/70 focus:ring-red-400/40' : 'border-white/25 focus:ring-sky-400/60 focus:border-sky-400'
                  }`}
                />
              </div>
            ...
            {/* <div>
              <label className="block text-sm font-medium text-sky-100/80 mb-1.5">Phone Number</label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-11 pr-4 py-3 border rounded-xl bg-white/10 text-white placeholder-gray-400 focus:outline-none focus:ring-2 transition ${
                    errors.phone ? 'border-red-400/70 focus:ring-red-400/40' : 'border-white/25 focus:ring-sky-400/60 focus:border-sky-400'
                  }`}
                />
              </div>
            ...
            <div>
              <label className="block text-sm font-medium text-sky-100/80 mb-1.5">Confirm Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-sky-300/70" />
                <input
                  ...
                  className={`w-full pl-11 pr-11 py-3 border rounded-xl bg-white/10 text-white placeholder-gray-400 focus:outline-none focus:ring-2 transition ${
                    errors.confirmPassword ? 'border-red-400/70 focus:ring-red-400/40' : 'border-white/25 focus:ring-sky-400/60 focus:border-sky-400'
                  }`}
                />
                <button
                  type="button"
            ...
            <label className="flex items-start gap-2 cursor-pointer mt-2">
              <input type="checkbox" className="w-4 h-4 mt-0.5 rounded border-sky-300/40 bg-sky-400/10 text-sky-400 focus:ring-sky-400/50 accent-sky-400" />
              <span className="text-sm text-sky-100/80">
                I agree to the{' '}
                <span className="text-sky-300 font-medium hover:underline">Terms of Service</span>
                {' '}and{' '}
                <span className="text-sky-300 font-medium hover:underline">Privacy Policy</span>
              </span>
            </label>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white font-semibold rounded-xl transition disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-sky-950/50 mt-2"
            >
